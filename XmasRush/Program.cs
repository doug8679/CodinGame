...
class GameBoard {
    ...
    public string FindPath(QuestItem curItem)
    {
        ...
        var canVert = (tile.Up && above.Down && nextVert == PathEnum.UP) || (tile.Down && below.Up && nextVert == PathEnum.DOWN);
        var canHorz = (tile.Left && left.Right && nextHorz == PathEnum.LEFT) || (tile.Right && right.Left && nextHorz == PathEnum.RIGHT);
    ...
    public string FindBestPush(QuestItem curItem) {
        ...
        var tile = Board[MyPosition[0], MyPosition[1]];
        if (xMoves < 0 && !tile.Left) {
            result = $"{MyPosition[0]-1} DOWN";
        } else if (xMoves > 0 && !tile.Right) {
            result = "{MyPosition[0]+1} DOWN";
        } else if (yMoves < 0 && !tile.Up) {
            result = "{MyPosition[1]-1} RIGHT";
        } else if (yMoves > 0 && !tile.Down) {
            result = "{MyPosition[1]+1} RIGHT";
        }
        return result;
    }
